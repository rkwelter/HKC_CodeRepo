function [nonlinear] = NonlinearUp(i,triples,uPlusModes)
%NONLINEARUP by Roland Welter
%   Writes the nonlinear terms for the uPlus variables
...
for i=1:numTrips
  ...
  if(coef0 ~= 0)
    ...
    if(coef1 ~= 0)
      ...
      % strcat('Zero coef for uP, m = (',num2str(p(1)),',',num2str(p(2)),') , p = (',num2str(q(1)),',',num2str(q(2)),') , q = (',num2str(m(1)),',',num2str(m(2)),')')
    end
  end

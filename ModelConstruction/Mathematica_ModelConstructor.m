...
if(strcmp(shear,'no'))
    title = strcat('HKC',num2str(M),'NoShear_Mathematica');
else
    title = strcat('HKC',num2str(M),'_Mathematica');
end

FID=fopen(strcat(title,'.m'),'w');
introString = [strcat('function [dX] = ',title,'(X,Pr,Ra,Ro,k1,V)'),newline,newline,'X = { '];
for i =1:numUp

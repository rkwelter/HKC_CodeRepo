%% LaTeX_ModelConstructor by Roland Welter 
% ModelConstructor builds the time stepper for a desired HKC spectral model
% such that the output can be read by LaTeX 
...
FID=fopen(strcat('../FluidSolver/TimeSteppers/',diffEq,'_LaTeX.m'),'w');
introString = [strcat('function [dX] = ',diffEq,'_LaTeX(X,Pr,Ra,Ro,k1,V)'),newline,newline,'\begin{equation} X = \begin{pmatrix} '];
for i =1:numUp
